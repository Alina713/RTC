...
class LinearETA(nn.Module):
    def __init__(self, config, data_feature):
        ...
        self.route_embedding = RouteEmbedding(d_model=self.d_model, dropout=0.1, 
                 add_pe=False, node_fea_dim = 2, add_gat=True, norm = True)
        self.model = nn.LSTM(input_size=self.d_model, hidden_size=self.d_model, batch_first=True, bidirectional=False)
    ...
    def forward(self, x, padding_masks, g, n_feat, travel_time, 
        ...
        route_input_embeds = self.route_embedding(x, g, n_feat)  # (B, T, d_model)

        h0 = torch.zeros(1, route_input_embeds.size()[0], self.d_model).cuda(1)
        c0 = torch.zeros(1, route_input_embeds.size()[0], self.d_model).cuda(1)

        token_emb,_ = self.model(route_input_embeds, (h0,c0))
