...
# 使用0填充到相同长度，并转换为Tensor
max_len = max(lengths)
train_trajectory_data = [x + [0] * (max_len - len(x)) for x in train_trajectory_data]
train_time_data = [x + [0] * (max_len - len(x)) for x in train_time_data]
X_train = torch.tensor(train_trajectory_data).float()
X_train = torch.narrow(X_train, dim=1, start=0, length=128)
y_train = torch.tensor(train_time_data).float()
y_train = torch.narrow(y_train, dim=1, start=0, length=128)
...
# 使用0填充到相同长度，并转换为Tensor
max_len = max(lengths)
valid_trajectory_data = [x + [0] * (max_len - len(x)) for x in valid_trajectory_data]
valid_time_data = [x + [0] * (max_len - len(x)) for x in valid_time_data]
X_valid = torch.tensor(valid_trajectory_data).float()
X_valid = torch.narrow(X_valid, dim=1, start=0, length=128)
y_valid = torch.tensor(valid_time_data).float()
y_valid = torch.narrow(y_valid, dim=1, start=0, length=128)
...
# 使用0填充到相同长度，并转换为Tensor
max_len = max(lengths)
test_trajectory_data = [x + [0] * (max_len - len(x)) for x in test_trajectory_data]
test_time_data = [x + [0] * (max_len - len(x)) for x in test_time_data]
X_test = torch.tensor(test_trajectory_data).float()
X_test = torch.narrow(X_test, dim=1, start=0, length=128)
y_test = torch.tensor(test_time_data).float()
y_test = torch.narrow(y_test, dim=1, start=0, length=128)
