...
void read() {
    ...
    if (file.is_open()) { // 判断文件是否成功打开
        ...
        while (getline(file, line)) { // 读取文件的每一行
            ...
            std::vector<double> row; // 创建每一行数据的向量
            double num;
            while (ss >> num) { // 将每个整数从字符串中提取并添加到行向量中
                row.push_back(num);
            }
            road_info.push_back(row);
            n++;
        }
        file.close(); // 关闭文件
    } else {
        std::cout << "无法打开路网文件。" << std::endl;
    }
    // cout << n <<endl;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // int id, p1, p2, c;
        int id = (int)road_info[i][0];
        int p1 = (int)road_info[i][1];
        int p2 = (int)road_info[i][2];
        int c = (int)road_info[i][3];
        // cout<<id<<";"<<p1<<":"<<p2<<";"<<c<<";"<<endl;
        // char way_string[50];
        ...
        // std::uniform_int_distribution<> dis(1, 100);  // 生成1到100之间的随机整数分布 
        // // 生成随机整数并输出, 85%概率保留邻接关系
        ...
        for (int j = 0; j < c; ++j) {
            // double x, y;
            // scanf("%lf%lf", &x, &y);
            double x = road_info[i][2*j+4];
            double y = road_info[i][2*j+5];
            Point* p = new Point();
            p->lat = x;
            p->lon = y;
    ...
    //     }
    // }
    std::ifstream trajfile("/Users/wyh/PycharmProjects/pythonProject2/validtraj_20150401_ShangHai.txt");
    // std::ifstream file("/Users/wyh/PycharmProjects/pythonProject2/edgeOSM.txt");
    // 因为经纬度为小数所以这里应为double而不是int
